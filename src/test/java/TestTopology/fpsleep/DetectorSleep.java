...
/**
 * Created by kailin on 17-10-16.
 */
public class DetectorSleep extends TASleepBolt implements Constant {

    private static final Logger LOG = LoggerFactory.getLogger(DetectorSleep.class);

    public DetectorSleep(IntervalSupplier sleep) {
        super(sleep);
    }
    ...
    public static class Entry implements Serializable {
        ...
        public String toString() {
            return "{count: "+count+" timestamp: "+timestamp+" refCount: "+refCount+" detectedBySelf: "+detectedBySelf+"}";
        }
    }
    ...
    public void execute(Tuple input) {
        ...
        wordListArrayList.forEach((pattern) -> {
            long temp = now;
            //TestRedis.insertList("timestamp" , String.valueOf(temp+":"+pattern));
            Entry entry = patterns.computeIfAbsent(pattern, (k) -> new Entry(temp));
